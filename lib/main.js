...
function reject( x, options, predicate, thisArg ) {
	...
	var i;
	if ( !isndarrayLike( x ) ) {
		throw new TypeError( format( '2555t', x ) );
	}
	...
	if ( !isFunction( clbk ) ) {
		throw new TypeError( format( '2552b', clbk ) );
	}
	if ( hasOpts ) {
		if ( !isPlainObject( opts ) ) {
			throw new TypeError( format( '2552V', opts ) );
		}
		...
		if ( hasOwnProp( opts, 'order' ) ) {
			if ( !isOrder( opts.order ) ) {
				throw new TypeError( format( '2555C', 'order', opts.order ) );
			}
			ord = opts.order;
		}
	} else {
	...
	if ( ctor === null ) {
		...
		throw new TypeError( format( '255Bf', 'dtype', opts.dtype ) );
	}
